<template>
  <div class="grid grid-cols-2 gap-2 mb-6">
    <div
      class="rounded-md shadow-md bg-gray-800 p-4 w-full transition-all duration-300 cursor-pointer"
      @click="toggle('male')"
    ...
    >
      ...
      <p class="text-center mt-8 uppercase font-bold">Мужчина</p>
    </div>
    <div
      class="rounded-md shadow-md bg-gray-800 p-4 w-full transition-all duration-300 cursor-pointer"
      @click="toggle('female')"
    ...
    >
      ...
      <p class="text-center mt-8 uppercase font-bold">Женщина</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    toggle(value) {
      this.gender = value
      this.$emit('genderListener', value)
    }
  }
};
</script>
